...
function get_kurucz_atmosphere(emulator, Teff::Real, logg::Real, Fe_H::Real, alpha_Fe::Real;
    ...
    # Extract variables and convert to Julia arrays
    # Note: PyTorch tensors need to be converted to numpy then to Julia
    tau = vec(atmosphere["TAU"].detach().cpu().numpy())
    T = vec(atmosphere["T"].detach().cpu().numpy())
    P = vec(atmosphere["P"].detach().cpu().numpy())
    RHOX = vec(atmosphere["RHOX"].detach().cpu().numpy())
    XNE = vec(atmosphere["XNE"].detach().cpu().numpy())
    ABROSS = vec(atmosphere["ABROSS"].detach().cpu().numpy())
    ACCRAD = vec(atmosphere["ACCRAD"].detach().cpu().numpy())
...
function kurucz_to_korg(kurucz_atm::KuruczAtmosphere; reference_wavelength::Real=5e-5)
    ...
    layers = Vector{Korg.PlanarAtmosphereLayer}(undef, n_layers)

    for i in 1:n_layers
        layers[i] = Korg.PlanarAtmosphereLayer(
            kurucz_atm.tau[i],        # tau_ref
            z[i],                      # z (cm)
            kurucz_atm.T[i],          # temp (K)
            kurucz_atm.XNE[i],        # electron_number_density (cm⁻³)
            number_density[i]          # number_density (cm⁻³)
        )
    end

    # Create Korg PlanarAtmosphere
    korg_atm = Korg.PlanarAtmosphere(layers, reference_wavelength)
